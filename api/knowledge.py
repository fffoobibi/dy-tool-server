...
@bp.post("/chat-with-workspace")
def chat_with_workspace():
    """知识库问答接口
    ---
    tags:
      - knowledge_base
    summary: 基于知识库内容进行智能问答
    description: 使用 OpenAI 兼容的 LLM 模型，结合知识库内容回答用户问题
    consumes:
      - application/json
    produces:
      - application/json
      ...
      - name: body
        ...
        schema:
          ...
          properties:
            chat_token:
              type: string
              description: OpenAI API 密钥或兼容服务的 token
              example: "sk-xxx"
            chat_url:
              type: string
              description: OpenAI API 基础地址或兼容服务的 URL
    ...
    responses:
      ...
      500:
        description: 服务器错误
    """
    chat_token = request.json.get("chat_token", "").strip()
    chat_url = request.json.get("chat_url", "").strip()
    chat_model = request.json.get("chat_model").strip()
    chat_question = request.json.get("chat_question").strip()

    if not chat_token or not chat_url:
        return fail(msg="缺少必要参数", code=400)
    chat = ChatOpenAI(
        model=chat_model,
        temperature=0.7,
        base_url=chat_url,
    ...
    prompts = [
        {
            "role": "system",
            "content": "你是个专业的助手，如果用户问你的问题，优先考虑知识库中的相关内容，你的回复语气要友好且专业，回答问题要像一个真实的人类口吻，不要太过于正式。",
        },
        {
            "role": "user",
    ...
    logger.info("Knowledge Base Chat Result: {}", result)
    return success(msg="success ", resp={"model": chat_model, "result": result})

    # try:
    #     service = get_knowledge_service()
    #     response = service.chat_with_workspace(token, chat_url)
    #     return success(msg="success", resp=response)
